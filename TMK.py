...
def menu_apikey():
  ...
  id = "→".join(uuid)
  server = requests.get(f'https://github.com/BLACKGANG1M/VAR/blob/main/AD.txt').text
...
VP   V8P  `Y88P'   `Y88P'  Y8888P'
 ...
 \033[1;32m└────────────────────────────────────────────────┘\n""")                                          
  ...
  try:
    httpCaht = requests.get(f"https://github.com/BLACKGANG1M/VAR/blob/main/AD.txt").text
    if id in httpCaht:
...
def ckx():
	uuid = str(os.geteuid()) + str(os.getlogin())
	id = "→".join(uuid)
	server = requests.get(f'https://github.com/BLACKGANG1M/VAR/blob/main/AD.txtt').text
	try:
		httpCaht = requests.get(f"https://github.com/BLACKGANG1M/VAR/blob/main/AD.txt").text
		if id in httpCaht:
			msg = str(os.geteuid())
